...
describe("getFibonacci()", function() {
  ...
  it("starting number should be smaller than first found/printed number", function() {
    let x = 5; // start
    let y = 4; // amount

    let fiboarr1 = [8, 13, 21, 34];
    let fiboarr2 = getFibonacci(x, y);
  ...
  it("should write file with function output as content", function() {
    ...
    chai
      .expect(fs.readFileSync("fibonacci_assignment.txt", "utf8"))
      .to.equal(fiboarr1);
  });
});
